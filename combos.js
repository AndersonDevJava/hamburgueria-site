  ...
  const combos = {
    ...
    // Verifica se existe o item para não duplicar na lista
  function addItemCar(produto) {
    ...
    // Função que recebe o item e exibição dinâmica
    ...
    function itensCarrinho(item){
      ...
      `
      // Lógica do CRUD do carrinho
      let quant = 1
      const contador = div.querySelector("h1") 
    ...
    itensMenu.forEach((item) => {
      ...
      limenu.className = 'bg-black p-3 combo-card list-none flex flex-col items-center gap-2'
      ...
      // Quando o usuário clica no título da categoria 
      limenu.querySelector("h3").addEventListener("click", () => {
        Object.keys(menu).forEach((itemMenu) => {
            // Verifica se o item clicado corresponde à categoria
          
            if(itemMenu === item.title){
            // Limpa estilos dos itens do menu
            document.querySelectorAll("#menu li").forEach(li => {
              li.style = "";
            });

            // Atualiza título e destaca o item clicado
            ...
            // Limpa e insere os produtos da categoria
            ...
            menu[itemMenu].forEach((produto, index) => {
              ...
              <button id="itens-menu-${index}" class="combo-card bg-red-900 text-center p-2 md:p-3 rounded-sm">adicionar ao carrinho</button>
              </div>
              </div>
              `
    ...
    return { saidamensagem, carrinho, renderPages }
  }